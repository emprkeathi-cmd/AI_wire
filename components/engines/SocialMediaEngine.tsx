...
export const SocialMediaEngine: React.FC<SocialMediaEngineProps> = ({ activeChat, currentTheme, onSendMessage, onDeleteMessage }) => {
  ...
  const posts = activeChat.messages.filter(m => 
    m.type === 'social_post' || 
    (m.role === 'assistant' && m.type === 'post' && activeChat.mode === 'social') ||
    (m.role === 'assistant' && (m.content.includes('http') || m.title) && activeChat.mode === 'social')
  );

  const handleAction = (post: Message, reaction: '✅' | '❌') => {
    const comment = comments[post.timestamp] || "";
    
    // 1. Send the data to n8n
    onSendMessage(comment ? `${reaction} Feedback: ${comment}` : `${reaction} Post Action`, 'reaction', {
    ...
    onDeleteMessage(post.timestamp);

    // 3. Cleanup local comment state
    setComments(prev => {
      const next = { ...prev };
      delete next[post.timestamp];
      return next;
    });
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar bg-[#02040a] p-4 sm:p-10">
      <div className="max-w-2xl mx-auto space-y-12 pb-32">
        ...
        {posts.length > 0 ? (
          posts.map((post) => (
            <div key={post.timestamp} className="bg-[#0f121d] border border-slate-800/60 rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
              {/* Card Header */}
              <div className="p-6 flex items-center justify-between border-b border-white/[0.03]">
                ...
                <button onClick={() => onDeleteMessage(post.timestamp)} className="p-2 text-slate-700 hover:text-red-500 transition-colors">
                  <Trash2 size={18} />
                </button>
              </div>
              ...
              <div className="p-6 bg-slate-950/40 border-t border-white/[0.03] space-y-4">
                 <div className="relative">
                   <textarea 
                     value={comments[post.timestamp] || ''}
                     onChange={(e) => setComments({ ...comments, [post.timestamp]: e.target.value })}
