...
export const AlarmEngine: React.FC<AlarmEngineProps> = ({ activeChat, currentTheme, onUpdateAlarms }) => {
  ...
  useEffect(() => {
    ...
    if (lastMsg?.role === 'assistant') {
      // Check if this message has already been processed to prevent re-triggering on reload
      const processedKey = `processed_msg_${activeChat.id}`;
      ...
      try {
        ...
        if (processed) {
          localStorage.setItem(processedKey, lastMsg.id);
        }
      } catch (e) {}
    }
