...
import { Chat, Alarm } from '@/src/types';
...
export const AlarmEngine: React.FC<AlarmEngineProps> = ({ activeChat, currentTheme, onUpdateAlarms }) => {
  ...
  const [ringingAlarmTimestamp, setRingingAlarmTimestamp] = useState<number | null>(null);
  ...
  // Handle incoming n8n signals (Timers only)
  useEffect(() => {
    const lastMsg = activeChat.messages[activeChat.messages.length - 1];
    if (lastMsg?.role === 'assistant') {
      const processedKey = `processed_timer_${activeChat.id}`;
      const lastProcessedId = localStorage.getItem(processedKey);
      if (lastProcessedId === String(lastMsg.timestamp)) return;
      ...
      try {
        ...
        if (data.timer_duration) {
          ...
          localStorage.setItem(processedKey, String(lastMsg.timestamp));
        }
      } catch (e) {}
    }
  ...
  const triggerAlarm = (alarm: Alarm) => {
    if (alarmAudioRef.current) {
      ...
      setIsRinging(true);
      setRingingAlarmTimestamp(alarm.timestamp);
    }
  };
  ...
  const stopAllSounds = () => {
    ...
    if (ringingAlarmTimestamp) {
      const alarm = activeChat.alarms?.find(a => a.timestamp === ringingAlarmTimestamp);
      if (alarm?.date) {
        const newAlarms = (activeChat.alarms || []).filter(a => a.timestamp !== ringingAlarmTimestamp);
        onUpdateAlarms(newAlarms);
      }
    }

    setIsRinging(false);
    setRingingAlarmTimestamp(null);
  };
  ...
  const toggleAlarm = (timestamp: number) => {
    const newAlarms = (activeChat.alarms || []).map(a => a.timestamp === timestamp ? { ...a, isActive: !a.isActive } : a);
    onUpdateAlarms(newAlarms);
  };

  const deleteAlarm = (timestamp: number) => {
    const newAlarms = (activeChat.alarms || []).filter(a => a.timestamp !== timestamp);
    onUpdateAlarms(newAlarms);
  };
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="space-y-3">
              {(activeChat.alarms || []).map(alarm => (
                <div key={alarm.timestamp} className="bg-slate-900/60 border border-slate-800/40 p-4 sm:p-6 rounded-[2rem] flex items-center justify-between group hover:border-slate-700 hover:bg-slate-900 transition-all shadow-lg">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={() => toggleAlarm(alarm.timestamp)}
                    ...
                    <button onClick={() => deleteAlarm(alarm.timestamp)} className="p-2 sm:p-3 text-slate-700 hover:text-red-500 transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100"><Trash2 size={18} /></button>
                  </div>
                </div>
              ))}
            </div>
