...
import { Chat, Message, ThemePalette } from '@/src/types';
...
interface CalendarEngineProps {
  ...
  onEditMessage: (timestamp: number, newContent: string, newCategoryId: string) => void; // Added edit prop
}
            ...
            >
              ...
              <div className="flex justify-between items-start mb-2 relative z-10">
                 ...
                 <button 
                   onClick={(e) => { 
                     ...
                     setEditingEventTimestamp(null); // Reset edit state
              ...
              <div className="space-y-1 relative z-10">
                {events.slice(0, 4).map(ev => {
                  ...
                  return (
                    <div key={ev.timestamp} className="text-[8px] font-black p-1 rounded bg-slate-950/60 truncate border border-slate-800/30 uppercase tracking-tighter" style={{ color: category?.color || '#94a3b8' }}>
                      • {ev.content}
                    </div>
                  );
                })}
              ...
              {(() => {
                ...
                return events.length > 0 ? (
                  events.map(ev => {
                    const category = activeChat.categories.find(c => c.id === ev.categoryId);
                    return (
                      <div key={ev.timestamp} className="p-6 bg-slate-800/50 rounded-[2rem] border border-slate-700/50 relative overflow-hidden group hover:border-slate-500 transition-colors shadow-sm">
                        ...
                        <button 
                          onClick={() => {
                            setEditingEventTimestamp(ev.timestamp);
                        ...
                        <button onClick={() => onDeleteMessage(ev.timestamp)} className="absolute right-4 top-4 p-2 text-slate-700 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16} /></button>
                      </div>
                    );
                  })
                ) : (
               ...
               <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3 text-white">
                  <div className={`p-3 ${currentTheme.bg} text-white rounded-2xl`}>
                    {editingEventTimestamp ? <Edit2 size={24} /> : <Plus size={24} />}
                  </div>
                  <h3 className="text-xl font-black uppercase tracking-tight">
                    {editingEventTimestamp ? 'Update Entry' : 'Direct Entry'}
                  </h3>
                </div>
                <button onClick={() => {
                  setIsEventModalOpen(false);
              ...
              <div className="space-y-6">
                ...
                <button 
                  onClick={() => {
                    ...
                    if (editingEventTimestamp) {
                      onEditMessage(editingEventTimestamp, inputText, selectedCategoryId || fallbackId);
                    } else {
                      onSendMessage(inputText, 'event', { date: selectedEventDate, categoryId: selectedCategoryId || fallbackId });
                    }
                    setEditingEventTimestamp(null); // Clear state
                ...
                >
                  {editingEventTimestamp ? 'Update Signal' : 'Commit Signal'}
                </button>
              </div>
            </div>
          </div>
