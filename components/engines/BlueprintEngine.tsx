...
interface BlueprintEngineProps {
  ...
  onUpdate: (timestamp: number, newContent: string) => void;
...
export const BlueprintEngine: React.FC<BlueprintEngineProps> = ({ activeChat, onDeploy, onUpdate, onDelete }) => {
  ...
  const [activeProjectTimestamp, setActiveProjectTimestamp] = useState<number | null>(null);
  ...
  const [blueprintChats, setBlueprintChats] = useState<Record<number, any[]>>({});
  ...
  const activeProject = useMemo(() => 
    activeChat.messages.find(m => m.timestamp === activeProjectTimestamp), 
    [activeProjectTimestamp, activeChat.messages]
  );
  ...
  useEffect(() => {
    if (activeProjectTimestamp) centerWorkspace();
  }, [activeProjectTimestamp]);
  ...
  const handleSendToUplink = async () => {
    ...
    const blueprintTimestamp = activeProject.timestamp;
    const msg = { id: Date.now(), role: 'user', content: uplinkInput, timestamp: Date.now() };
    setBlueprintChats(prev => ({ ...prev, [blueprintTimestamp]: [...(prev[blueprintTimestamp] || []), msg] }));
    const userPrompt = uplinkInput;
    setUplinkInput('');
    setIsAIProcessing(true);
    try {
      if (activeChat.webhookUrl) {
        // Updated to send full project context (title, content, and message) as requested
        const response = await sendMessageToN8N(activeChat.webhookUrl, userPrompt, 'blueprint_chat', { 
          blueprintTimestamp, 
          title: activeProject.title,
        ...
        setBlueprintChats(prev => ({ ...prev, [blueprintTimestamp]: [...(prev[blueprintTimestamp] || []), aiMsg] }));
      }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10">
          {activeChat.messages.filter(m => m.type === 'blueprint').map(bp => (
            <div 
              ...
              onClick={() => { setActiveProjectTimestamp(bp.timestamp); setIsEditing(false); setIsSyncOpen(false); }}
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex items-center gap-2 sm:opacity-0 group-hover/card:opacity-100 transition-all">
                  <button onClick={(e) => { e.stopPropagation(); setActiveProjectTimestamp(bp.timestamp); setIsSyncOpen(true); }} className="p-3 text-slate-600 hover:text-emerald-400 bg-slate-900/60 rounded-[1.2rem] border border-slate-800 transition-colors shadow-lg"><MessageCircle size={18} /></button>
                  <button onClick={(e) => { e.stopPropagation(); onDelete(bp.timestamp); }} className="p-3 text-slate-600 hover:text-red-500 bg-slate-900/60 rounded-[1.2rem] border border-slate-800 transition-colors"><Trash2 size={18} /></button>
                </div>
              </div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                ...
                <button onMouseDown={e => e.stopPropagation()} onClick={() => setShowCodex(!showCodex)} className={`p-2.5 rounded-xl transition-all ${showCodex ? 'bg-amber-600 text-white' : 'text-slate-500 hover:text-white'}`} title="Protocol Codex"><HelpCircle size={18} /></button>
                  <button onMouseDown={e => e.stopPropagation()} onClick={() => setIsSyncOpen(!isSyncOpen)} className={`p-2.5 rounded-xl transition-all ${isSyncOpen ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:text-white'}`} title="Neural Uplink"><MessageCircle size={18} /></button>
                <button onMouseDown={e => e.stopPropagation()} onClick={() => setActiveProjectTimestamp(null)} className="p-2.5 hover:bg-white/10 rounded-xl text-slate-500 hover:text-red-500 transition-all"><X size={20} /></button>
              </div>
            </div>
            ...
            <div className="flex-1 flex overflow-hidden bg-[#05060a] relative">
                ...
                {isEditing ? (
                  <div className="flex-1 flex overflow-hidden animate-in fade-in duration-120">
                    ...
                    <div className="flex-1 p-6 sm:p-12">
                      <textarea 
                        value={activeProject.content} onChange={(e) => onUpdate(activeProject.timestamp, e.target.value)}
              ...
              <div 
                className={`absolute top-0 right-0 bottom-0 w-full sm:w-[320px] bg-[#0a0d16] border-l border-slate-800/90 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] flex flex-col z-20 transition-transform duration-200 transform ${isSyncOpen ? 'translate-x-0' : 'translate-x-full'}`}
              >
                ...
                <div ref={chatScrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 custom-scrollbar select-text">
                  ...
                  {(blueprintChats[activeProject.timestamp] || []).map((msg) => (
              ...
              {/* PROTOCOL CODEX DRAWER */}
              <div 
                className={`absolute top-0 right-0 bottom-0 w-full sm:w-[280px] bg-[#0d111a] border-l border-slate-800/90 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] flex flex-col z-30 transition-transform duration-200 transform ${showCodex ? 'translate-x-0' : 'translate-x-full'}`}
              >
                <div className="h-12 flex items-center justify-between px-6 bg-[#1a1f35] border-b border-slate-800/40 shrink-0">
                  <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white">Protocol Codex</span>
                  <button onClick={() => setShowCodex(false)} className="text-slate-500 hover:text-white transition-all"><ChevronRight size={18} /></button>
                </div>
                <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 italic">Formatting</h4>
                    <div className="space-y-3">
                      <div className="flex items-center gap-3 text-[11px] font-bold text-slate-300"><span className="w-10 bg-slate-900 p-1.5 rounded-lg text-center text-[9px] text-indigo-400 border border-slate-800">+++</span> Master Title</div>
                      <div className="flex items-center gap-3 text-[11px] font-bold text-slate-300"><span className="w-10 bg-slate-900 p-1.5 rounded-lg text-center text-[9px] text-indigo-400 border border-slate-800">++</span> Section Head</div>
                      <div className="flex items-center gap-3 text-[11px] font-bold text-slate-300"><span className="w-10 bg-slate-900 p-1.5 rounded-lg text-center text-[9px] text-indigo-400 border border-slate-800">+</span> Paragraph Head</div>
                      <div className="flex items-center gap-3 text-[11px] font-bold text-slate-300"><span className="w-10 bg-slate-900 p-1.5 rounded-lg text-center text-[9px] text-slate-500 border border-slate-800">-</span> Metadata Tag</div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600 italic">Neural Links</h4>
                    <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800/60">
                      <div className="text-emerald-400 font-mono text-[10px] mb-2">[[Label|Index]]</div>
                      <p className="text-[9px] text-slate-500 font-bold leading-relaxed uppercase tracking-widest">Creates a hoverable reference to an asset. Replace 'Index' with the asset number.</p>
                    </div>
                  </div>
                </div>
              </div>
