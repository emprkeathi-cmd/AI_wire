...
interface NewsEngineProps {
  activeChat: Chat;
  currentTheme: any;
  onUpdateMessage: (timestamp: number, updates: Partial<Message>) => void;
...
export const NewsEngine: React.FC<NewsEngineProps> = ({ activeChat, currentTheme, onUpdateMessage, onDeleteMessage }) => {
  ...
  const markAsRead = (post: Message) => {
    const quizComplete = !post.quiz || post.quiz.length === 0 || post.quiz.every(q => q.isLocked);
    if (!quizComplete) {
      if (navigator.vibrate) navigator.vibrate(50);
      alert("Neural sync required. Please lock in all quiz answers.");
      return;
    }
    onUpdateMessage(post.timestamp, { isRead: true });
    setActiveIndex(0);
    if (navigator.vibrate) navigator.vibrate(30);
  };

  const handleSelectAnswer = (postTimestamp: number, questionId: string, answer: string) => {
    const post = activeChat.messages.find(m => m.timestamp === postTimestamp);
    if (!post || !post.quiz) return;

    const newQuiz = post.quiz.map(q => {
      if (q.id === questionId && !q.isLocked) {
        return {
          ...q,
          userAnswer: answer
        };
    ...
    onUpdateMessage(postTimestamp, { quiz: newQuiz });
    if (navigator.vibrate) navigator.vibrate(10);
  };

  const handleLockIn = (postTimestamp: number, questionId: string) => {
    const post = activeChat.messages.find(m => m.timestamp === postTimestamp);
    if (!post || !post.quiz) return;

    const newQuiz = post.quiz.map(q => {
      if (q.id === questionId && q.userAnswer !== undefined && !q.isLocked) {
        return {
          ...q,
          isLocked: true,
            ...
            <div className="w-full max-w-2xl h-full flex flex-col bg-[#0f121d] border border-slate-800/60 rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.6)] overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <div className="p-8 sm:p-10 flex items-start justify-between bg-gradient-to-b from-white/[0.03] to-transparent border-b border-white/[0.02]">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={() => onDeleteMessage(activePost.timestamp)} className="p-3 text-slate-700 hover:text-red-500 transition-colors"><Trash2 size={20} /></button>
                </div>
              </div>
                     ...
                     {activePost.quiz.map((q, idx) => (
                       <div key={q.id} className="space-y-6">
                         <div className="flex justify-between items-center">
                           ...
                           {q.userAnswer && !q.isLocked && (
                             <button 
                               onClick={() => handleLockIn(activePost.timestamp, q.id)}
                               className="px-4 py-1.5 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-500/20"
                             >
                               Lock In
                             </button>
                           )}
                           ...
                           {q.options.map((opt) => {
                             ...
                             return (
                               <button 
                                 key={opt}
                                 disabled={isLocked}
                                 onClick={() => handleSelectAnswer(activePost.timestamp, q.id, opt)}
                               ...
                               >
                                 ...
                                 {isLocked && isSelected && (isCorrect ? <Check size={16} /> : <X size={16} />)}
                                 {!isLocked && isSelected && <Circle size={14} className="text-indigo-400 fill-indigo-400" />}
                                 {!isSelected && !isLocked && <Circle size={14} className="opacity-0 group-hover:opacity-100" />}
                               </button>
                             );
                           })}
                         </div>
              ...
              {activePost.quiz && activePost.quiz.length > 0 && (
                <div className="h-2 bg-slate-950">
                  <div 
                    ...
                    style={{ width: `${(activePost.quiz.filter(q => q.isLocked).length / activePost.quiz.length) * 100}%` }}
                  />
                </div>
              )}
            </div>
