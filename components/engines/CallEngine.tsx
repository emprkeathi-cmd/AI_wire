...
export const CallEngine: React.FC<CallEngineProps> = ({ activeChat, currentTheme, onUpdateSettings, onSendMessage }) => {
  ...
  const lastProcessedMsgTimestampRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    if (lastMsg?.role === 'assistant' && lastMsg.timestamp !== lastProcessedMsgTimestampRef.current) {
      lastProcessedMsgTimestampRef.current = lastMsg.timestamp;
