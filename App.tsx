  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden relative select-none">
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto px-2 space-y-1">
            <div className="px-3 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
              <Zap size={14} className={currentTheme.text} />
              Neural Links
            </div>
            {chats.map((chat, idx) => (
              <div key={chat.id} className="relative group">
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl transition-all flex items-center gap-3 mb-1 pr-12 ${
                    activeChatId === chat.id 
                      ? `bg-gradient-to-br ${currentTheme.from}/30 ${currentTheme.to}/10 text-white ring-1 ${currentTheme.ring}` 
                ...
                >
                  <div className={`p-0.5 rounded-xl overflow-hidden w-10 h-10 flex items-center justify-center ${activeChatId === chat.id ? `bg-gradient-to-br ${currentTheme.from} ${currentTheme.to} shadow-lg ${currentTheme.shadow} text-white` : 'bg-slate-800'}`}>
                    {chat.icon ? (
                      <img src={chat.icon} alt={chat.name} className="w-full h-full object-cover rounded-lg" />
                    ) : (
                      <Bot size={20} />
                    )}
                  </div>
                  <div className="flex-1 truncate">
                    ...
                    <div className="text-[10px] text-slate-500 truncate mt-0.5 font-medium">
                      {chat.messages.length > 0 ? chat.messages[chat.messages.length - 1].content : 'Awaiting sync'}
                    </div>
                  </div>
                </button>
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={(e) => { e.stopPropagation(); moveChat(idx, 'up'); }} className="p-1 hover:bg-slate-700 rounded text-slate-500" disabled={idx === 0}>
                    <ChevronUp size={14} />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); moveChat(idx, 'down'); }} className="p-1 hover:bg-slate-700 rounded text-slate-500" disabled={idx === chats.length - 1}>
                    <ChevronDown size={14} />
                  </button>
                </div>
              </div>
      ...
      <main className="flex-1 flex flex-col h-full bg-slate-950">
        <header className="h-16 flex items-center justify-between px-4 border-b border-slate-800 bg-slate-900/40 backdrop-blur-xl sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 text-slate-400 hover:bg-slate-800 rounded-lg transition-colors">
              <Menu size={22} />
            </button>
            {activeChat && (
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 bg-gradient-to-tr ${currentTheme.from} ${currentTheme.to} rounded-2xl flex items-center justify-center text-white font-black shadow-lg ${currentTheme.shadow} transform rotate-3 overflow-hidden`}>
                   {activeChat.icon ? (
                     <img src={activeChat.icon} className="w-full h-full object-cover -rotate-3" alt="" />
                   ) : (
                     <div className="-rotate-3">{activeChat.name.charAt(0)}</div>
                   )}
                </div>
                <h2 className="font-bold text-sm tracking-tight">{activeChat.name}</h2>
              </div>
            )}
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => setIsChatSettingsOpen(true)} className="p-2.5 text-slate-400 hover:bg-slate-800 rounded-xl transition-all">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {!activeChatId ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-6 text-center">
              ...
              <div>
                <h3 className="text-2xl font-black text-white italic tracking-tighter">AI wire</h3>
                <p className="max-w-[240px] mx-auto text-sm text-slate-500 mt-2 font-medium uppercase tracking-widest">Neural Command Center</p>
              </div>
            </div>
          ) : (
            activeChat?.messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'system' ? (
                  ...
                  <div className={`max-w-[88%] lg:max-w-[75%] px-5 py-4 rounded-[2rem] shadow-2xl relative group transition-all ${
                    msg.role === 'user' 
                      ? `bg-gradient-to-br ${currentTheme.from} ${currentTheme.to} text-white rounded-tr-none` 
                      : 'bg-slate-900 border border-slate-800/50 text-slate-100 rounded-tl-none'
                  }`}>
                    {msg.type === 'audio' ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                           <span className="text-sm font-bold block">Encrypted Log</span>
                           <span className="text-[10px] opacity-70 uppercase tracking-widest font-bold">Secure Path</span>
                        </div>
                      </div>
                    ) : (
        ...
        {activeChatId && (
          <footer className="p-4 border-t border-slate-800 bg-slate-900/60 backdrop-blur-2xl">
            <div className="max-w-4xl mx-auto flex items-end gap-3 relative">
              {(isPreparingRecord || isRecording) && (
                <div className="absolute inset-0 bg-slate-900 z-50 flex items-center px-4 rounded-3xl border border-slate-700/50 shadow-2xl animate-in fade-in zoom-in duration-200">
                  {isPreparingRecord && (
                    <div className={`absolute top-0 left-0 h-1 ${currentTheme.bg} transition-all duration-75`} style={{ width: `${holdProgress}%` }} />
                  )}
                  {isRecording && (
                    <button onClick={cancelRecording} className="p-3 mr-2 bg-slate-800 text-red-500 rounded-xl hover:bg-slate-700 transition-all border border-slate-700">
                      <Trash2 size={20} />
                    </button>
                  )}
                  <div className="flex flex-1 items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      <span className="font-black text-[10px] uppercase tracking-widest text-slate-400">
                        {isRecording ? 'Capturing Frequency' : 'Initiating Link...'}
                      </span>
                      <span className="font-mono text-xl text-white font-bold leading-none">
                        {isRecording ? formatTime(recordingTime) : `${( (HOLD_DURATION - (holdProgress * HOLD_DURATION / 100)) / 1000 ).toFixed(1)}s`}
                      </span>
                    </div>
                  </div>
                  {isRecording && (
                    <button onClick={stopRecording} className={`ml-auto px-5 py-2.5 bg-gradient-to-br ${currentTheme.from} ${currentTheme.to} text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg ${currentTheme.shadow} hover:scale-105 active:scale-95 transition-all flex items-center gap-2`}>
                      Deploy <Send size={14} />
                    </button>
                  )}
                </div>
              ...
              <button 
                ...
                className={`p-4 rounded-2xl transition-all duration-300 transform active:scale-90 touch-none ${
                  isRecording 
                  ? `${currentTheme.bg} text-white scale-110 shadow-2xl ${currentTheme.shadow}` 
                  : isPreparingRecord 
                  ? `${currentTheme.bg} text-white scale-105 animate-pulse`
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  className="w-full bg-slate-800 text-slate-100 rounded-2xl px-5 py-4 pr-14 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-slate-700 border border-slate-700/50 transition-all resize-none min-h-[56px] max-h-[160px] shadow-inner"
                  rows={1}
                />
                <button 
                  ...
                  className={`absolute right-2 bottom-2 p-3 rounded-xl transition-all ${
                    inputText.trim() && !isTyping 
                      ? `bg-gradient-to-br ${currentTheme.from} ${currentTheme.to} text-white hover:scale-105 shadow-lg ${currentTheme.shadow}` 
                      : 'text-slate-600'
                  }`}
                >
                  <Send size={20} />
            ...
            <div className="text-[9px] text-center text-slate-600 mt-3 font-black uppercase tracking-[0.3em] opacity-40">
              Hold mic 1.7s for neural uplink • Tap deploy to sync
            </div>
          </footer>
        )}
      </main>
            ...
            <div className="p-8">
              <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-black uppercase tracking-tight">Agent Sync</h3>
                </div>
                <button onClick={() => setIsChatSettingsOpen(false)} className="p-2 text-slate-500 hover:text-white transition-colors">
                  <X size={28} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div onDragOver={(e) => e.preventDefault()} onDrop={handleIconDrop} className="w-full h-32 border-2 border-dashed border-slate-700 rounded-3xl flex flex-col items-center justify-center gap-2 hover:border-slate-500 transition-colors cursor-pointer relative overflow-hidden group">
                    {activeChat.icon ? (
                      <>
                        <img src={activeChat.icon} className="absolute inset-0 w-full h-full object-cover opacity-40" alt="" />
                        <div className="relative z-10 flex flex-col items-center text-white">
                          <Upload size={24} className="mb-1" />
                          <span className="text-[10px] font-bold uppercase text-center px-4">Update Neural Representation</span>
                        </div>
                      </>
                    ) : (
                      <>
                        <ImageIcon size={32} className="text-slate-600" />
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Drop Representation</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Designation</label>
                  <input type="text" value={activeChat.name} onChange={(e) => updateActiveChat({ name: e.target.value })} className="w-full bg-slate-800 border border-slate-700/50 rounded-2xl px-5 py-3.5 focus:outline-none font-bold text-sm" />
                </div>

                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">n8n Uplink Webhook</label>
                  <input type="url" value={activeChat.webhookUrl} onChange={(e) => updateActiveChat({ webhookUrl: e.target.value })} className="w-full bg-slate-800 border border-slate-700/50 rounded-2xl px-5 py-3.5 focus:outline-none font-mono text-[10px] tracking-tight" />
                </div>

                <button onClick={() => deleteChat(activeChat.id)} className="w-full flex items-center justify-between px-5 py-4 rounded-2xl bg-red-950/20 hover:bg-red-950/40 text-red-500 border border-red-900/20 transition-all text-xs font-bold uppercase tracking-widest mt-4">
                  Sever Link <Trash2 size={16} />
                </button>
              </div>
            </div>
            <div className="bg-slate-800/30 p-6 flex justify-end">
              <button onClick={() => setIsChatSettingsOpen(false)} className={`px-8 py-3 ${currentTheme.bg} text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg ${currentTheme.shadow}`}>
                Save Sync Data
              </button>
            </div>
          </div>
      ...
      {/* Global Settings Modal - With Palette Selection */}
      {isSettingsModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-slate-800 p-8 shadow-2xl">
            ...
            <div className="space-y-8">
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-6 flex items-center gap-2">
                  <Palette size={14} /> Interface Color Palette
                </label>
                  ...
                  {(Object.keys(paletteConfigs) as ThemePalette[]).map((p) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-full transition-all flex items-center justify-center ring-offset-4 ring-offset-slate-900 ${
                        palette === p ? `ring-2 ${paletteConfigs[p].ring} scale-110` : 'hover:scale-110'
                      }`}
                      style={{ backgroundColor: paletteConfigs[p].hex }}
                    >
                      {palette === p && <Zap size={16} className="text-white" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-800">
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest leading-relaxed">
                  System Revision: 2.1.0-Neural<br/>
                  Tunnel Integrity: Secure<br/>
                  Latency Optimization: Active
                </p>
              </div>

              <button onClick={() => setIsSettingsModalOpen(false)} className={`w-full py-4 bg-white text-slate-950 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-200 transition-all`}>
