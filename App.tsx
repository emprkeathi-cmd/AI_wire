...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (content?: string, type: 'text' | 'audio' | 'event' | 'file' = 'text', eventData?: {date: string, categoryId: string}, blobToUse?: Blob) => {
    const messageContent = content || inputText;
    const finalBlob = blobToUse || audioBlob;
    if (!messageContent && !finalBlob && type !== 'file') return;

    const userMessage: Message = {
      id: Date.now().toString(),
      role: 'user',
      content: type === 'audio' ? 'Voice Message' : String(messageContent),
      type,
      date: eventData?.date,
    ...
    if (activeChat?.webhookUrl) {
      setIsTyping(true);
      try {
        const response = await sendMessageToN8N(activeChat.webhookUrl, type === 'audio' ? finalBlob! : String(messageContent), type);
                ...
                <>
                  {activeChat?.mode === 'chat' && (
                    <>
                      ...
                      <button onMouseDown={handleMicMouseDown} onMouseUp={handleMicMouseUp} onMouseLeave={handleMicMouseUp} onContextMenu={(e) => e.preventDefault()} className={`p-4 bg-slate-800 text-slate-400 hover:text-white rounded-2xl shrink-0 transition-all active:scale-95 ${holdProgress > 0 ? 'bg-slate-700 text-white' : ''}`}><Mic size={24} className={holdProgress > 0 ? 'animate-pulse' : ''} /></button>
                    </>
                  )}
                  <div className="flex-1 relative min-w-0">
                    <textarea 
                      ...
                      rows={1} 
                    />
                    <button onClick={() => handleSendMessage()} disabled={!inputText.trim() || isTyping} className={`absolute right-2 bottom-2 p-3 rounded-xl transition-all ${inputText.trim() && !isTyping ? `${currentTheme.bg} text-white shadow-lg` : 'text-slate-600'}`}><Send size={20} /></button>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="relative bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl flex flex-col max-h-[92dvh] animate-in zoom-in-95 duration-200">
            <div className="p-6 sm:p-8 overflow-y-auto flex-1 custom-scrollbar text-white">
              ...
              <div className="space-y-8">
                ...
                {/* 3. Mode Specific (Classifications for Calendar) */}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[110] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in slide-in-from-bottom-4 duration-300">
            <div className="p-6 sm:p-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 backdrop-blur-md">
               <div>
                 <h3 className="text-lg sm:text-xl font-black uppercase tracking-tight italic text-white">{new Date(selectedEventDate).toLocaleDateString('default', { month: 'long', day: 'numeric', year: 'numeric' })}</h3>
