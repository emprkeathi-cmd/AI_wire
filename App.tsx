...
const App: React.FC = () => {
  ...
  const processDoorMessage = useCallback((raw: string, chat: Chat): { messages: Message[], debug: any } => {
    ...
    try {
      ...
      // START SPECIFIC USER FORMAT PARSING: Month DD, YYYY Category Content
      ...
      // END SPECIFIC USER FORMAT PARSING
  ...
  const handleSendMessage = async (
    ...
    const userMessage: Message = {
      ...
      type: finalType,
      date: eventData?.date || (activeChat.mode === 'calendar' ? new Date().toISOString().split('T')[0] : undefined),
  ...
  const handleMicMouseDown = (e?: React.MouseEvent | React.TouchEvent) => {
    // Basic long-press visual feedback
    setHoldProgress(1);
  ...
  const handleMicMouseUp = (e?: React.MouseEvent | React.TouchEvent) => {
    ...
    setHoldProgress(0);
    // If user releases early and we were recording, we should ideally stop it
    if (isRecording) {
      stopRecording(true);
    }
  };
    ...
    <div className="flex h-[100dvh] w-full bg-slate-950 text-slate-100 overflow-hidden relative select-none">
      ...
      {isDeployModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-lg rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl p-6 sm:p-10 animate-in zoom-in-95 duration-200">
