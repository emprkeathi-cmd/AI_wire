...
const App: React.FC = () => {
  ...
  const processDoorMessage = useCallback((raw: string, chat: Chat): { messages: Message[], debug: any } => {
    ...
    try {
      ...
      if (foundCat && (foundDate || dateStr)) {
        ...
        if (finalContent) {
          messages.push({ id: `seq-${Date.now()}`, role: 'assistant', content: finalContent, date: foundDate || dateStr, categoryId: foundCat.id, type: 'event', timestamp: Date.now() });
  ...
  const handleSendMessage = async (content?: string, type: 'text' | 'audio' | 'event' | 'file' | 'reaction' = 'text', eventData?: {date: string, categoryId: string}, blobToUse?: Blob, metadata?: any) => {
    const messageContent = content || inputText;
    const finalBlob = blobToUse || audioBlob;
    if (!messageContent && !finalBlob && type !== 'file' && type !== 'reaction') return;

    const userMessage: Message = {
      id: Date.now().toString(),
      role: 'user',
      content: type === 'audio' ? 'Voice Message' : type === 'reaction' ? `Reaction: ${content}` : String(messageContent),
      type,
      date: eventData?.date,
    ...
    // Only add to chat UI if it's not a background reaction signal
    if (type !== 'reaction') {
    ...
    if (activeChat?.webhookUrl) {
      setIsTyping(true);
      try {
        const response = await sendMessageToN8N(activeChat.webhookUrl, type === 'audio' ? finalBlob! : String(messageContent), type, metadata);
        if (type !== 'reaction' && response) {
  ...
  const handleReaction = (msg: Message, emoji: string) => {
    handleSendMessage(emoji, 'reaction', undefined, undefined, { originalContent: msg.content });
  ...
  const exportNeuralCore = () => {
    const data = { chats, palette, timestamp: Date.now() };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `ai_wire_core_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  ...
  const stopRecording = (shouldSend = false) => {
    ...
    } else if (shouldSend && audioBlob) {
      handleSendMessage(undefined, 'audio', undefined, audioBlob);
    }
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-slate-950 text-slate-100 overflow-hidden relative select-none">
      ...
      <main className="flex-1 flex flex-col h-[100dvh] bg-slate-950 min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 relative custom-scrollbar flex flex-col">
          {!activeChatId ? (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start mb-2 relative z-10">
                         ...
                         <button 
                           ...
                           className="p-1 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-slate-400 hover:text-white rounded-lg touch-manipulation"
                         >
                           <Plus size={14} />
                         </button>
                      </div>
            ...
            <div className="space-y-6 max-w-4xl mx-auto w-full flex-1 pt-4 pb-12">
                  ...
                  {msg.role === 'system' ? (
                    ...
                    <div className="relative group max-w-[85%] lg:max-w-[75%]">
                      <div 
                        ...
                        className={`px-5 py-4 rounded-[2rem] shadow-2xl relative transition-all cursor-pointer active:scale-[0.98] ${msg.role === 'user' ? `bg-gradient-to-br ${currentTheme.from} ${currentTheme.to} text-white rounded-tr-none` : 'bg-slate-900 border border-slate-800/50 text-slate-100 rounded-tl-none hover:border-slate-700'}`}
                      >
                      ...
                      {reactionTargetId === msg.id && (
                        <div className="absolute -top-12 left-0 flex items-center gap-2 bg-slate-800/90 backdrop-blur-xl p-1.5 rounded-2xl border border-white/10 shadow-2xl animate-in fade-in slide-in-from-bottom-2 z-50">
                          <button onClick={() => handleReaction(msg, '✅')} className="p-2 hover:bg-emerald-500/20 text-emerald-400 rounded-xl transition-all"><CheckCircle2 size={20} /></button>
                          <button onClick={() => handleReaction(msg, '❌')} className="p-2 hover:bg-rose-500/20 text-rose-400 rounded-xl transition-all"><XCircle size={20} /></button>
                        </div>
                      )}
                    </div>
                  )}
        ...
        {activeChatId && (
          <footer className="p-4 border-t border-slate-800 bg-slate-900/60 backdrop-blur-2xl relative shrink-0">
            ...
            <div className="max-w-4xl mx-auto flex items-end gap-3 relative">
              {(activeChat?.mode === 'chat') && (isRecording || audioBlob) ? (
                <div className={`flex-1 flex items-center justify-between p-2 rounded-2xl bg-slate-800 border border-slate-700 h-[56px] animate-in slide-in-from-bottom-2`}>
                  <button onClick={() => { setAudioBlob(null); setIsRecording(false); }} className="p-3 text-red-500 hover:bg-red-500/10 rounded-xl transition-all touch-manipulation"><Trash2 size={24} /></button>
                  ...
                  <button onClick={() => stopRecording(true)} className={`p-3 ${currentTheme.bg} text-white rounded-xl shadow-lg active:scale-95 transition-all touch-manipulation`}><Send size={24} /></button>
                </div>
              ) : (
                <>
                  {activeChat?.mode === 'chat' && (
                    <>
                      ...
                      <button onClick={() => chatFileInputRef.current?.click()} className="p-4 bg-slate-800 text-slate-400 hover:text-white rounded-2xl shrink-0 transition-colors touch-manipulation"><Paperclip size={24} /></button>
                      <button 
                        ...
                        className={`p-4 bg-slate-800 text-slate-400 hover:text-white rounded-2xl shrink-0 transition-all active:scale-95 touch-manipulation ${holdProgress > 0 ? 'bg-slate-700 text-white' : ''}`}
                      >
                  ...
                  <div className="flex-1 relative min-w-0">
                    <textarea 
                      ...
                      rows={1} 
                    />
                    <button onClick={() => handleSendMessage()} disabled={!inputText.trim() || isTyping} className={`absolute right-2 bottom-2 p-3 rounded-xl transition-all ${inputText.trim() && !isTyping ? `${currentTheme.bg} text-white shadow-lg` : 'text-slate-600'} touch-manipulation`}><Send size={20} /></button>
                  </div>
                </>
              )}
            </div>
      ...
      {isChatSettingsOpen && editingChat && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl flex flex-col max-h-[92dvh] animate-in zoom-in-95 duration-200">
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-6">
                      <div className={`w-24 h-24 rounded-3xl bg-slate-800 border-2 border-slate-700 overflow-hidden flex items-center justify-center relative shadow-inner aspect-square`}>
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <button onClick={() => iconInputRef.current?.click()} className="w-full py-3 px-4 bg-slate-800 border border-slate-700 hover:border-slate-500 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 touch-manipulation">
                ...
                {editingChat.mode === 'calendar' && (
                  <div>
                    ...
                    }} className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-700 transition-all flex items-center justify-center gap-2 touch-manipulation"><Plus size={14} /> Extend Labels</button>
                  </div>
                )}
                ...
                <div className="flex flex-col gap-3 pt-4 pb-4">
                  <button onClick={saveSettings} className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl active:scale-95 transition-all ${currentTheme.bg} text-white touch-manipulation`}>Commit Protocol Changes</button>
                  <button onClick={() => deleteChat(editingChat.id)} className="w-full py-2 rounded-xl text-red-500/60 hover:text-red-500 transition-all text-[10px] font-black uppercase tracking-[0.2em] touch-manipulation">Sever Protocol</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-w-lg rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl p-6 sm:p-10 animate-in zoom-in-95 duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <button onClick={() => createNewAgent('chat')} className="flex flex-col items-center gap-4 sm:gap-6 p-6 sm:p-8 bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-purple-500/50 rounded-[2rem] sm:rounded-[2.5rem] transition-all group active:scale-95 shadow-xl touch-manipulation">
              ...
              <button onClick={() => createNewAgent('calendar')} className="flex flex-col items-center gap-4 sm:gap-6 p-6 sm:p-8 bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-emerald-500/50 rounded-[2rem] sm:rounded-[2.5rem] transition-all group active:scale-95 shadow-xl touch-manipulation">
      ...
      {isSettingsModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-slate-800 p-8 shadow-2xl animate-in zoom-in-95 duration-200 text-white overflow-y-auto max-h-[90dvh]">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-5 justify-center">
                  {(Object.keys(paletteConfigs) as ThemePalette[]).map((p) => (
                    <button key={p} onClick={() => setPalette(p)} className={`w-14 h-14 rounded-full transition-all flex items-center justify-center ring-offset-4 ring-offset-slate-900 touch-manipulation ${palette === p ? `ring-2 ${paletteConfigs[p].ring} scale-110 shadow-2xl shadow-${palette}-500/50` : 'hover:scale-110'}`} style={{ backgroundColor: paletteConfigs[p].hex }}>{palette === p && <Zap size={20} className="text-white" />}</button>
                  ))}
                </div>
              </div>
              
              <div className="bg-slate-950 p-6 rounded-3xl border border-slate-800 space-y-4">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-2 flex items-center gap-2"><Download size={14} /> Neural Backup</label>
                <button onClick={exportNeuralCore} className="w-full py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-2 border border-slate-700 active:scale-95 touch-manipulation">
                  Download Neural Core (.json)
                </button>
                <p className="text-[8px] text-slate-600 font-bold uppercase tracking-tight text-center">Save your entire chat and node memory to your device storage.</p>
              </div>

              <button onClick={() => setIsSettingsModalOpen(false)} className="w-full py-4 bg-white text-slate-950 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-200 transition-all active:scale-95 shadow-lg touch-manipulation">Finalize HUD</button>
            </div>
          </div>
        </div>
      )}
