...
import { 
  ...
  Clock as AlarmIcon, Phone, Newspaper, Instagram
} from 'lucide-react';
...
const App: React.FC = () => {
      ...
      try {
        ...
        if (payload.event === 'message' && payload.message) {
          ...
          if (newMessages.length > 0 && "Notification" in window && Notification.permission === "granted") {
            new Notification(`[${activeChat.name}] New Signal`, {
              body: rawMsg.length > 100 ? rawMsg.substring(0, 97) + "..." : rawMsg,
              icon: activeChat.icon || 'https://cdn-icons-png.flaticon.com/512/2103/2103830.png'
            });
          }
  ...
  const handleSendMessage = async (content?: string, type: any = 'text', extra?: any, blobToUse?: Blob, fileToUse?: File) => {
    ...
    const userMessage: Message = {
      ...
      categoryId: extra?.categoryId || (activeChat.mode !== 'chat' && activeChat.mode !== 'blueprint' && activeChat.mode !== 'alarm' && activeChat.mode !== 'call' ? activeChat.categories[0]?.id : undefined),
      timestamp: Date.now(),
    ...
    if (activeChat?.webhookUrl) {
      ...
      try {
        ...
        else if (finalType === 'event') metadata = { ...metadata, date: userMessage.date, categoryId: userMessage.categoryId, categoryName: activeChat.categories.find(c => c.id === userMessage.categoryId)?.name };
        else if (finalType === 'task') metadata = { ...metadata, todoStatus: userMessage.todoStatus, todoReminder: userMessage.todoReminder, todoNotes: userMessage.todoNotes, categoryId: userMessage.categoryId, categoryName: activeChat.categories.find(c => c.id === userMessage.categoryId)?.name };
  ...
  const updateAlarms = (alarms: Alarm[]) => {
    setChats(prev => prev.map(c => c.id === activeChatId ? { ...c, alarms } : c));
  };

  const updateCallSettings = (settings: { threshold: number; silenceTimeout: number }) => {
    setChats(prev => prev.map(c => c.id === activeChatId ? { ...c, callSettings: settings } : c));
  };
  ...
  const createNewAgent = (mode: 'chat' | 'calendar' | 'todo' | 'blueprint' | 'alarm' | 'call' | 'news' | 'social') => {
    ...
    const modeNames = { chat: 'Neural Link', calendar: 'Temporal Node', todo: 'Task Grid', blueprint: 'Blueprint Hub', alarm: 'Alarm Module', call: 'Voice Loop', news: 'News Feed', social: 'Social Grid' };
    const newChat: Chat = { 
      id: newId, 
