            ...
            activeChat?.messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'system' ? (
                  ...
                  <div className={`max-w-[88%] lg:max-w-[75%] px-5 py-4 rounded-[2rem] shadow-2xl relative group transition-all ${msg.role === 'user' ? `bg-gradient-to-br ${currentTheme.from} ${currentTheme.to} text-white rounded-tr-none` : 'bg-slate-900 border border-slate-800/50 text-slate-100 rounded-tl-none'}`}>
                    {msg.type === 'audio' ? (
        ...
        {activeChatId && (
          <footer className="p-4 border-t border-slate-800 bg-slate-900/60 backdrop-blur-2xl">
            <div className="max-w-4xl mx-auto flex items-end gap-3 relative">
              ...
              <button onPointerDown={handleMicDown} onPointerUp={handleMicUp} onPointerLeave={handleMicUp} className={`p-4 rounded-2xl transition-all duration-300 transform active:scale-90 touch-none ${isRecording ? `${currentTheme.bg} text-white scale-110 shadow-2xl ${currentTheme.shadow}` : isPreparingRecord ? `${currentTheme.bg} text-white scale-105 animate-pulse` : 'bg-slate-800 text-slate-400 hover:text-white'}`}>
      ...
      {isChatSettingsOpen && activeChat && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-4">
          ...
          <div className="relative bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl">
                  ...
                  <div onDragOver={(e) => e.preventDefault()} onDrop={handleIconDrop} className="w-full h-32 border-2 border-dashed border-slate-700 rounded-3xl flex flex-col items-center justify-center gap-2 hover:border-slate-500 transition-colors cursor-pointer relative overflow-hidden group">
                    {activeChat.icon ? (
                      <>
                        ...
                        <div className="relative z-10 flex flex-col items-center text-white"><Upload size={24} className="mb-1" /><span className="text-[10px] font-bold uppercase text-center px-4">Update Neural Representation</span></div>
                      </>
                    ) : (
                      <>
            ...
            <div className="bg-slate-800/30 p-6 flex justify-end"><button onClick={() => setIsChatSettingsOpen(false)} className={`px-8 py-3 ${currentTheme.bg} text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg ${currentTheme.shadow}`}>Save Sync Data</button></div>
          </div>
        </div>
      )}
      {isSettingsModalOpen && (
