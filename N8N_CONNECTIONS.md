...
### B. Task Grid (Todo)
Use the `command` key to manage the list. The engine is fuzzy and accepts multiple field aliases:
```json
{
  "command": "save",      // Aliases: "WHAT", "what" | Values: "save", "create", "delete", "done", "complete"
  "category": "Today",    // Alias: "Category" | Must match a node category name
  "title": "Buy milk",    // Aliases: "Title", "content", "Content", "appointment", "Appointment"
  "notes": "Get 2%",      // Optional: Extra details
  "reminder": true        // Optional: boolean or "positive" string
}
```
- **Delete**: Set `command` to `"delete"`.
- **Complete**: Set `command` to `"done"` or `"complete"`.
...
### H. Neural Sync Engine (Logs)
Post status updates or synchronization logs to the Sync Node:
```json
{
  "type": "sync_update",
  "content": "Neural Bridge established. All nodes are now in parity." // Alias: "text"
}
```

---

## 4. Neural Cleanup & Deduplication

The AI Wire system includes a **Neural Cleaner** that automatically regulates data integrity.

### Automatic Regulation
- **Trigger**: Whenever the **Sync** button is pressed in the app, a Neural Cleanup scan is performed immediately before the outbound webhook is triggered.
- **Logic**: It scans the current node for duplicate entries based on:
    - **Calendar**: Same Date + Title + Category.
    - **Todo**: Same Title + Category + Notes.
    - **News/Social**: Same Title + Content.

### Manual Regulation
Users can trigger a manual scan via the **Neural Cleanup** button in the Sync Engine UI to purge accidental copies without triggering a full sync.

---

## 5. Tips for n8n Success
