...
export const useBlueprintEngine = (
  ...
  const handleUpdateBlueprint = useCallback((msgId: string, newContent: string, chatId?: string) => { 
  ...
  const handleDeleteMessage = useCallback(async (msgId: string, chatId?: string) => { 
    const targetId = chatId || activeChatId;
    const targetChat = chats.find(c => c.id === targetId);
    if (!targetChat) return;

    const message = targetChat.messages.find(m => m.id === msgId);
  ...
  const processBlueprintMessage = useCallback((data: any): { additions: Message[], debug: any } | null => {
    if (data.type === 'blueprint' || data.sync_type === 'full_recreation_payload') {
      return {
        additions: [{ 
          id: `blueprint-recreation-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`, 
          role: 'assistant', 
          type: 'blueprint',
