...
export const useTodoEngine = (
  ...
  const toggleTaskStatus = useCallback(async (timestamp: number, currentStatus: 'active' | 'done' | 'deleted', chatId?: string) => {
    ...
    const task = targetChat.messages.find(m => m.timestamp === timestamp);
    
    setChats(prev => prev.map(c => c.id === targetId ? { ...c, messages: c.messages.map(m => m.timestamp === timestamp ? { ...m, todoStatus: nextStatus } : m) } : c));
  ...
  const deleteTask = useCallback(async (timestamp: number, chatId?: string) => {
    const targetId = chatId || activeChatId;
    const targetChat = chats.find(c => c.id === targetId);
    if (!targetChat) return;
    ...
    setChats(prev => prev.map(c => c.id === targetId ? { ...c, messages: c.messages.map(m => m.timestamp === timestamp ? { ...m, todoStatus: 'deleted' } : m) } : c));
  ...
  const editTask = useCallback(async (timestamp: number, updates: any, chatId?: string) => {
    const targetId = chatId || activeChatId;
    const targetChat = chats.find(c => c.id === targetId);
    if (!targetChat) return;
    ...
    if (jsonData && typeof jsonData === 'object') {
      ...
      if (cat && title) {
        if (cmd === 'save' || cmd === 'create') {
          return {
            additions: [{ role: 'assistant', type: 'task', content: String(title), categoryId: cat.id, todoStatus: 'active', todoReminder: jsonData.reminder === 'positive' || jsonData.reminder === true, todoNotes: String(jsonData.notes || ''), timestamp: Date.now() }],
            deletions: [],
            updates: [],
