      ...
      if (raw.includes('|')) {
        ...
        if (parts.length >= 3) {
          ...
          if (potentialDate && potentialCat) {
            return {
              additions: [{ id: `sep-${Date.now()}`, role: 'assistant', content: parts.slice(2).join('|').trim(), date: potentialDate, categoryId: potentialCat.id, type: 'event', timestamp: Date.now() }],
              deletions: [],
              updates: [],
      ...
      if (explicitCatValue && explicitContent) {
        ...
        if (cat) {
          if (action === 'delete') {
            ...
            return {
              additions: [{ id: `expl-${Date.now()}`, role: 'assistant', content: String(explicitContent), date: currentDateStr, categoryId: cat.id, type: 'event', timestamp: Date.now() }],
              deletions: [],
              updates: [],
