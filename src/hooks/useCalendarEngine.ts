...
export const useCalendarEngine = (
  ...
  const editCalendarEvent = useCallback(async (id: string, newContent: string, newCategoryId: string, chatId?: string) => {
    const targetId = chatId || activeChatId;
    const targetChat = chats.find(c => c.id === targetId);
    if (!targetChat) return;

    const event = targetChat.messages.find(m => m.id === id);
  ...
  const deleteCalendarEvent = useCallback(async (id: string, chatId?: string) => {
    const targetId = chatId || activeChatId;
    const targetChat = chats.find(c => c.id === targetId);
    if (!targetChat) return;
  ...
  const processCalendarMessage = useCallback((
      ...
      if (raw.includes('|')) {
        ...
        if (parts.length >= 3) {
          ...
          if (potentialDate && potentialCat) {
            return {
              additions: [{ id: `sep-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`, role: 'assistant', content: parts.slice(2).join('|').trim(), date: potentialDate, categoryId: potentialCat.id, type: 'event', timestamp: Date.now() }],
              deletions: [],
              updates: [],
    ...
    if (data && typeof data === 'object') {
      ...
      const action = String(data.command || data.WHAT || data.what || 'create').toLowerCase();
        ...
        if (cat) {
          if (action === 'delete') {
            ...
            return {
              additions: [],
              deletions: [],
              updates: [{ content: String(explicitContent), date: currentDateStr, categoryId: cat.id }],
              debug: { ...debugInfo, interpreted: { type: 'calendar_json_edit', category: cat.name, date: currentDateStr } }
            };
          } else {
            return {
              additions: [{ id: `expl-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`, role: 'assistant', content: String(explicitContent), date: currentDateStr, categoryId: cat.id, type: 'event', timestamp: Date.now() }],
              deletions: [],
              updates: [],
